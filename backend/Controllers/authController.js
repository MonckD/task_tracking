...
const ALLOWED_REGISTER_ROLES = ['developpeur', 'designer', 'graphiste'];

async function register(req, res) {
    try {
        const { nom, email, telephone, password, role } = req.body;

        const finalRole = ALLOWED_REGISTER_ROLES.includes(role) ? role : 'developpeur';
...
async function createAdmin(req, res) {
    try {
        ...
        res.status(201).json({
            user: { id: user.id, nom: user.nom, email: user.email, is_admin: user.is_admin },
        });
    } catch (error) {
...
export { register, login, createAdmin, profile };